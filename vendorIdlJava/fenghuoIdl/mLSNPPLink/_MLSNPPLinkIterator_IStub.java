...
public class _MLSNPPLinkIterator_IStub
	extends org.omg.CORBA.portable.ObjectImpl
	implements mLSNPPLink.MLSNPPLinkIterator_I
{
	private String[] ids = {"IDL:mtnm.tmforum.org/mLSNPPLink/MLSNPPLinkIterator_I:1.0"};
	public String[] _ids()
	{
		return ids;
	}

	public final static java.lang.Class _opsClass = mLSNPPLink.MLSNPPLinkIterator_IOperations.class;
	public int getLength() throws globaldefs.ProcessingFailureException
	{
		...
		else
		{
			...
			MLSNPPLinkIterator_IOperations _localServant = (MLSNPPLinkIterator_IOperations)_so.servant;
			int _result;			try
			{
	...
	public boolean next_n(int how_many, mLSNPPLink.MLSNPPLinkList_THolder mLSNPPLink) throws globaldefs.ProcessingFailureException
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				...
				mLSNPPLink.value = MLSNPPLinkList_THelper.read(_is);
				return _result;
			}
			catch( org.omg.CORBA.portable.RemarshalException _rx ){}
			catch( org.omg.CORBA.portable.ApplicationException _ax )
		...
		else
		{
			...
			boolean _result;			try
			{
			_result = _localServant.next_n(how_many,mLSNPPLink);
			}
			finally
			{
				_servant_postinvoke(_so);
