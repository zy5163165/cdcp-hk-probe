...
public final class AlarmFilterTask_THelper
{
	...
	public static extendedEmsMgr.AlarmFilterTask_T read (final org.omg.CORBA.portable.InputStream in)
	{
		...
		result.alarmFilterTaskName = globaldefs.NVSList_THelper.read(in);
		result.entityName = globaldefs.NVSList_THelper.read(in);
		...
		result.emsBeginTime=in.read_string();
	...
	{
		globaldefs.NVSList_THelper.write(out,s.alarmFilterTaskName);
		globaldefs.NVSList_THelper.write(out,s.entityName);
		...
		out.write_string(s.emsBeginTime);
