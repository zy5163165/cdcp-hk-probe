...
public class _AlarmReportingMaskIterator_IStub
	extends org.omg.CORBA.portable.ObjectImpl
	implements extendedEmsMgr.AlarmReportingMaskIterator_I
{
	private String[] ids = {"IDL:mtnm.tmforum.org.fiberhome.extended/extendedEmsMgr/AlarmReportingMaskIterator_I:1.0"};
	public String[] _ids()
	{
		return ids;
	}

	public final static java.lang.Class _opsClass = extendedEmsMgr.AlarmReportingMaskIterator_IOperations.class;
	public boolean next_n(int how_many, extendedEmsMgr.AlarmReportingMaskList_THolder alarmReportingMaskList) throws globaldefs.ProcessingFailureException
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				...
				alarmReportingMaskList.value = extendedEmsMgr.AlarmReportingMaskList_THelper.read(_is);
				return _result;
			}
		...
		else
		{
			...
			AlarmReportingMaskIterator_IOperations _localServant = (AlarmReportingMaskIterator_IOperations)_so.servant;
			boolean _result;			try
			{
			_result = _localServant.next_n(how_many,alarmReportingMaskList);
			}
			finally
			{
