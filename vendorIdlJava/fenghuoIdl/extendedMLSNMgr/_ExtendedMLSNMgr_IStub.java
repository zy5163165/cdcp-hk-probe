	...
	implements extendedMLSNMgr.ExtendedMLSNMgr_I
{
	...
	public void deleteTNetworkProtectionGroups(globaldefs.NameAndStringValue_T[] tnProtectionGroupName) throws globaldefs.ProcessingFailureException
	{
		...
		if(! this._is_local())
		{
			...
			try
			{
				...
				globaldefs.NVSList_THelper.write(_os,tnProtectionGroupName);
				_is = _invoke(_os);
				return;
			}
			catch( org.omg.CORBA.portable.RemarshalException _rx ){}
	...
	public void getMLSNBackBoneRoute(globaldefs.NameAndStringValue_T[] subnetName, int how_many, boolean getMainRoute, topologicalLink.TopologicalLinkList_THolder routeList, topologicalLink.TopologicalLinkIterator_IHolder topoIt) throws globaldefs.ProcessingFailureException
	{
		while(true)
		{
		if(! this._is_local())
		{
			...
			try
			{
				...
				globaldefs.NVSList_THelper.write(_os,subnetName);
				_os.write_ulong(how_many);
				_os.write_boolean(getMainRoute);
				_is = _invoke(_os);
				routeList.value = topologicalLink.TopologicalLinkList_THelper.read(_is);
		...
		else
		{
			...
			try
			{
			_localServant.getMLSNBackBoneRoute(subnetName,how_many,getMainRoute,routeList,topoIt);
			}
			finally
			{
				_servant_postinvoke(_so);
